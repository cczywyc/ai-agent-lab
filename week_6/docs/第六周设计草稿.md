...
> 状态：**定稿 v0.2**（周二设计日产出，六个决策已锁定，待周三验证、周四实现）
> 基线：`search_agent v3.0`（week_4&5）　目标：`v4.0`（状态化工作流）
> 本文不含实现代码——代码是周四的事，今天产出的是**决策 + 状态图**。
...
**per-query 标志的重置（决策 C）**：`has_*`、`*_injected`、`consecutive_failures`、`turn_count` 在 v3.0 里是 `run_agent` 的局部变量、每次调用天然清零。加 `checkpointer` 后 state 跨轮持久化，不重置就会串到下个问题。解法：开头的 **`init` 节点**显式重置这些标志（连带把 `retrieved_chunks` 清空）。这就是踩坑 #2"时序"思维的图上版本——从隐式依赖函数调用边界，变成图入口的显式重置。
...
| 节点 | 对应 v3.0 代码 | 干的活 |
|---|---|---|
| `init` | （新增）`run_agent` 局部变量初始化 | 重置本轮 per-query 标志（`has_*` / `*_injected` / `consecutive_failures` / `turn_count`）+ 清空 `retrieved_chunks` |
...
```mermaid
flowchart TD
    START([START]) --> init["init<br/>重置标志 + 清空 chunks"]
...
1. State schema 用 `add_messages` 单 reducer + 其余替换，跑最小三节点图（`assemble → agent → finalize`）能否正常传状态？
2. `init` 重置标志 + 清空 `retrieved_chunks`，配合 checkpointer 持久化，能否做到"跨轮记 messages 历史、但 per-query 状态清零"？（验证决策 C）
3. `retrieved_chunks` 在 `tools` 节点手动"当前+新"累加、`init` 返回 `[]` 清空：一个问题内多次检索能正确累加、下个问题能干净重置？引用校验能命中本问题任意一次检索的 chunk？（验证决策 B）
4. 纠正 cycle（`inject_correction_* → agent`）与 `turn_count` 闸门配合，不死循环？（复刻防护限制）
5. `human_review` 在 `finalize` 前调 `interrupt()`，`INTERRUPT_ENABLED=False` 时是否完全透明（不暂停）？（验证决策 F 的开关）
6. （可选，为日后 Store 迁移攒证据）LangGraph `Store` 的语义索引召回质量 vs E2c 的 namespace 方案。
...
| C | per-query 标志重置 | 入口 `init` 节点显式重置（含 `retrieved_chunks` 清空） |
...
*定稿 v0.2，2026-06-02。基于 search_agent v3.0；周三验证、周四照此实现 v4.0。*
