...
> 状态：**v0.3**（周四实现并入：真实 FastMCP server + stdio + client 站端到端跑通，`verify_thursday` **11/11**；决策 A/B/C/D/E/G 真实化坐实，**决策 F 出发现型——真实 SDK 把两层错误塌成 `isError`、§五"两层分开报"假设被证伪、host 派发前重建二分**；见各节 `[v0.3]` 标记 + `search_agent_mcp/`）。v0.2 = 周三桩测（E1–E6，run_all 6/6 + pytest 16/16）。v0.1 = 周二设计日初稿。下一站周五：真实模型驱动 + 端到端复证。
...
## 决策定稿表（v0.3，A–G，状态=桩测 E1–E6 + 周四实现 `verify_thursday` 11/11 并入）
