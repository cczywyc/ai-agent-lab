...
# ============================================================
# v6.0 多 Agent 桩：supervisor 拆 1 个研究子任务、critic 恒 accept、writer 直接成稿、reviewer 恒 accept
# （S9/S11/S12/S14 是 store 端到端测试，不验多 Agent 路由——把各 worker 钉成最简，让
#  supervisor→researcher→writer→reviewer 退化为"单研究子任务 + 一次过评审"，复用原 store 端到端断言。）
# ============================================================

def stub_planner_single(_messages):
    return '["执行本步调研"]'   # supervisor 拆成单个研究子任务（不触发 should_have_* 纠正）


def stub_critic_accept(_messages):
    return "VERDICT: accept"


def stub_writer_echo(messages):
    """writer 桩：把收到的 findings 投影（user 段）原样回成初稿——保留 researcher 结论 + 引用，
    好让 update_memory 仍能从交付稿里抽到带合法引用的事实（store 白名单契约 e2e 不变）。"""
    user = " ".join(m.get("content", "") for m in messages if m.get("role") == "user")
    return f"初稿：{user}"


def stub_reviewer_accept(_messages):
    return "VERDICT: accept\nSCORE: 0.9\nNOTES: 通过"
...
def test_s9_graph_e2e():
    ...
    real_planner, real_critic = nodes.call_supervisor_model, nodes.call_critic_model
    real_writer, real_reviewer = nodes.call_writer_model, nodes.call_reviewer_model
    ...
    try:
        ...
        nodes.call_supervisor_model = stub_planner_single   # 每问题拆 1 个子任务
        nodes.call_critic_model = stub_critic_accept     # 单步恒 accept
        nodes.call_writer_model = stub_writer_echo
        nodes.call_reviewer_model = stub_reviewer_accept
        ...
        check("最终调研报告含末步结论", "结论：阈值是 2。" in result["answer"])
    finally:
        nodes.call_model = real_call
        memory_pkg.get_memory = real_get
        nodes.execute_tool = real_exec
        nodes.call_supervisor_model = real_planner
        nodes.call_critic_model = real_critic
        nodes.call_writer_model = real_writer
        nodes.call_reviewer_model = real_reviewer
...
def test_s11_empty_answer_not_recorded():
    ...
    try:
        memory_pkg.get_memory = lambda: mgr
        nodes.call_supervisor_model = stub_planner_single
        nodes.call_critic_model = stub_critic_accept   # 被空回答硬闸门覆盖（escalate）
        nodes.call_writer_model = stub_writer_echo
        nodes.call_reviewer_model = stub_reviewer_accept
        # 模型恒返回空 content 的 stop（qwen 偶发抖动的复刻）；外循环最终 max_replan 收口
        ...
        check("空回答报告为占位符前缀", result["answer"].startswith("[模型返回空回答]"))
...
def test_s12_memory_records_post_review_answer():
    ...
    try:
        memory_pkg.get_memory = lambda: mgr
        nodes.call_supervisor_model = stub_planner_single
        nodes.call_critic_model = stub_critic_accept
        nodes.call_writer_model = stub_writer_echo
        nodes.call_reviewer_model = stub_reviewer_accept
        nodes.call_model = lambda msgs: SimpleNamespace(choices=[SimpleNamespace(
        ...
        check("暂停时草稿（报告）含执行器产出", "__interrupt__" in mid
              and "草稿回答" in mid["__interrupt__"][0].value.get("draft_answer", ""))
...
def test_s14_fact_contract_e2e():
    ...
    try:
        memory_pkg.get_memory = lambda: mgr
        nodes.call_supervisor_model = stub_planner_single
        nodes.call_critic_model = stub_critic_accept   # 仅对"引用合法的干净稿"放行
        nodes.call_writer_model = stub_writer_echo
        nodes.call_reviewer_model = stub_reviewer_accept
        ...
        clean = "- 降级在连续失败 2 次后触发 [设计笔记#降级机制]。"
        clean_stop = SimpleNamespace(choices=[SimpleNamespace(
            message=SimpleNamespace(content=clean, tool_calls=None), finish_reason="stop")])
        responses = iter([tool_resp, dirty_stop, clean_stop])
        ...
        check("真实来源的事实进了 store（重做稿被 accept 后入库）",
              any("降级在连续失败" in f for f in facts), str(facts))
        check("编造来源的句子未入库（critic 引用闸门 + extractor 白名单双保险）",
