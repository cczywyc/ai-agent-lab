...
> 状态：**v0.2**（桩测 E1–E7 已实跑 **7/7 全过**，结果并入；决策 A–H 全获实证，三处并入见各节 `[v0.2]` 标记）。v0.1 = 周二设计日初稿。
> 基线：`search_agent v5.0`（week_7，planner-executor-critic 外循环）　目标：`v6.0`（supervisor 多 Agent）
> 本文不含实现代码——代码是周四的事，本文产出的是**决策 + 状态图 + 已验证清单（桩测 7/7 已并入）**。
...
## 决策定稿表（v0.2，A–H，状态=桩测 E1–E7 已实跑 7/7 全过）
...
## 已验证清单（桩测 E1–E7，已实跑 7/7，详见 `experiment/实验结论.md`）
...
*v0.2，2026-06-12（桩测 E1–E7 实跑日并入）。v0.1 = 周二设计日初稿（决策 A–H 已锁，A=B（supervisor）、E=`MAX_REVIEW=2`）。本版并入桩测 7/7 全过结果：三处 `[v0.2]` 标记（§一 A′ prebuilt 非零 API、§四 F 隔离=显式投影、§五 E4 写读同 key + E7 归零属继承）+ 决策定稿表/已验证清单更新。真实跑策略缺陷修复留周末并入 v0.x。实测环境 LangGraph 1.2.4 / Python 3.12.3（`.venv`）。*
*设计前提见《第八周概念认知参考答案》；实验计划见 `experiment/第八周实验计划.md`、**实跑结论见 `experiment/实验结论.md`（E1–E7，7/7）**；基线见 week_7 `docs/第七周设计草稿.md`（v0.6）。证据来源（周一/周二检索）：Anthropic 多 Agent 研究系统博客（orchestrator-worker、委派契约四要素、共享上下文/强依赖不适合）、OpenAI Agents SDK 文档（manager-as-tools vs handoff、handoff 即 transfer_to_X 工具、input_filter）、LangGraph supervisor/swarm（`create_supervisor`、`Command(goto, graph=Command.PARENT)`、先 supervisor 后 swarm、track handoff count / recursion guard）、reflection 循环实践（2–3 轮足够、早退、Reflexion verbal reinforcement、behavioral collapse 取 best-so-far）。*
