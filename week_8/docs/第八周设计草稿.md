...
**成本账**：researcher（继承 v5.0 RAG 引擎）独占 79–89% token（A/B/C 79–85%、D 88.8%），v6.0 新增 writer+reviewer 打回层仅 3.8–12.1%——**多 Agent 串接没炸开成本**，贵的是内层检索（老问题）。但 4 题合计 **~103 万 token 一个 session 烧穿免费额度**（qwen3.7-max 跑完 A/B/C 即 403、D 改用旧模型残量）——评测/复跑要算额度账。
