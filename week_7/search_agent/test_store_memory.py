...
# ============================================================
# v5.0 外循环桩：planner 拆 1 个子任务、critic 恒 accept
# （S9/S11/S12/S14 是 store 端到端测试，不验外循环路由——把 planner/critic 钉成最简，
#  让 planner-executor-critic 退化为"单步执行"，复用原 v4.2 端到端断言。）
# ============================================================

def stub_planner_single(_messages):
    return '["执行本步调研"]'   # 拆成单个子任务（不触发 should_have_* 纠正）
...
def test_s9_graph_e2e():
    ...
    real_planner, real_critic = nodes.call_planner_model, nodes.call_critic_model
    ...
    try:
        ...
        nodes.call_planner_model = stub_planner_single   # 每问题拆 1 个子任务
        ...
        nodes.call_planner_model = real_planner
...
def test_s11_empty_answer_not_recorded():
    ...
    try:
        memory_pkg.get_memory = lambda: mgr
        nodes.call_planner_model = stub_planner_single
